...
class DisplayUserProfileForWebView(APIView):
    ...
    def get(self, request):
        ...
        # print(serializer.data[id(id)])
        # print(serializer.data['availability'])
...
class DisplayUserInCompany(APIView):
    ...
    def get(self, request):
        ...
        for i in company:
            ...
            # print(i.company_user.username)
            company_name.append(i.company_user.username)
            company_id.append(i.company_user.id)
            print(i.id)
        users_ = UserProfile.objects.all()
        serializer = DisplayUserProfileSerializer(users_, many=True)
        for i in range(len(company_name)):
            jsonformat['companies'] = {company_id[i]: {
                'userprofile': serializer.data[i],
